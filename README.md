...
## Engineering notes (things that broke and why)

Standing up a real Hadoop + Spark cluster on a memory-constrained
machine surfaced several concrete issues, each fixed rather than worked
around:

- **Bitnami deprecated free Spark images on Docker Hub** mid-project
  (`bitnami/spark:3.3` returns 404) — switched to `bitnamilegacy/spark:3.3`.
- **Spark's `UnifiedMemoryManager` has a hard ~450MB floor** per JVM
  (driver or executor) — the initial memory budget split across master +
  2 workers didn't leave enough per-process headroom; rebalanced to
  bigger allocations on fewer workers rather than starving all of them.
- **The upstream `bde2020/hadoop-nodemanager` image ships no Python**,
  but NodeManager is where mapper/reducer subprocesses actually execute —
  built a custom image (`docker/nodemanager.Dockerfile`) installing
  Python 3.5 from the Debian 9 archive (stretch is EOL; had to repoint
  `sources.list` at `archive.debian.org`).
- **YARN needs explicit config to run Hadoop Streaming at all**:
  `HADOOP_MAPRED_HOME` in the AM/map/reduce task environment (otherwise
  the ApplicationMaster can't find its own classes), and
  `yarn.nodemanager.aux-services=mapreduce_shuffle` (otherwise every
  task fails with `InvalidAuxServiceException`).
- **That NodeManager's Python is 3.5** — no f-strings. All Streaming
  mapper/reducer scripts use `.format()` for portability.

## Limitations / how this scales

This runs single-node pseudo-distributed Hadoop and a 1-worker Spark
cluster because the dev machine has 8GB total RAM. Everything here is
parameterized to scale up given more resources:

- `docker compose up --scale spark-worker=N` for more Spark parallelism
- Add more `datanode`/`nodemanager` service replicas for real HDFS/YARN
  data distribution
- `data/download_dataset.py --month` accepts any TLC month; concatenate
  several for a larger input and re-run to see the crossover point where
  distributed overhead stops dominating
